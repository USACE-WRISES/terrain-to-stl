...
def write_shell_stl(
    ...
    try:
        ...
        print_interval = max(1, total_row_pairs // 20) if total_row_pairs > 0 else 1
        callback_interval = progress_interval(total_row_pairs)
        ...
        for row_pair_index, row_pair_triangles in iter_raster_triangles_by_row_pair(
            ...
            completed_row_pairs = row_pair_index + 1
            if (
                total_row_pairs > 0
                and (
                    completed_row_pairs == total_row_pairs
                    or completed_row_pairs == 1
                    or completed_row_pairs % callback_interval == 0
                )
            ):
                emit_progress(
                    progress_callback,
                    ...
                    (
                        "Writing bottom and top STL surfaces "
                        f"({completed_row_pairs}/{total_row_pairs} row pairs)..."
                    ),
                )

            if (
                total_row_pairs > 0
                and ((row_pair_index + 1) % print_interval == 0 or row_pair_index + 1 == total_row_pairs)
            ):
                print(f"  processed {row_pair_index + 1}/{total_row_pairs} raster row pairs")

        if total_row_pairs == 0:
            emit_progress(
                progress_callback,
                "write-surfaces",
                1,
